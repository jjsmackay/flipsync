...
const INPUT_CLASS =
  'w-full border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm dark:bg-gray-900 dark:text-gray-100 disabled:opacity-50'
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {knobs.map((knob) => {
        ...
        if (knob.kind === 'checkbox') {
          return (
            <label
              ...
              htmlFor={id}
              title={knob.hint}
              className="flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-400 cursor-pointer select-none self-end pb-1.5"
            >
              <input
                id={id}
                type="checkbox"
        ...
        return (
          <div key={knob.key} title={knob.hint}>
