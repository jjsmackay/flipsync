...
import type { ScoutStatus, SpeakerCandidate, PoolTurn } from '../../types/api'
import {
  ApiError,
  startScout,
  getScoutStatus,
...
export const SCOUTABLE_STATUSES = new Set([
  'diarisation_pending',
  'diarisation_running',
  'diarisation_failed',
  'complete',
...
interface SpeakerScanPickerProps {
  projectId: string
  // Source id a scan/rescan targets. Empty string disables auto-scan and
  // "Scan again" (no scoutable source to run against yet).
  autoSourceId: string
  onSelected: () => void
  pollIntervalMs?: number
  // Whether to fire a scan automatically when no scout result exists yet.
  autoScan?: boolean
}

export function SpeakerScanPicker({
  projectId,
  autoSourceId,
  onSelected,
  pollIntervalMs = 3000,
  autoScan = true,
}: SpeakerScanPickerProps) {
  ...
  async function beginScan(sourceId: string, expected?: number) {
    setFindError(null)
    try {
      await startScout(projectId, sourceId, expected)
  ...
  useEffect(() => {
    if (!autoScan || !scoutLoaded || scout != null || !autoSourceId) return
    if (autoFired.current) return
    autoFired.current = true
    void beginScan(autoSourceId)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoScan, scoutLoaded, scout, autoSourceId])
  ...
  const { error: pollError } = usePolling<ScoutStatus>(() => getScoutStatus(projectId), {
  ...
  async function handleSelect(label: string, excludedIndices: number[]) {
    ...
    try {
      await selectScoutSpeaker(projectId, label, excludedIndices)
      onSelected()
    } catch (err) {
