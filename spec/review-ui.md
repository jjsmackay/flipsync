...
7. **Models** (v1.5, XTTS) — directly below Pipeline; rendered only when XTTS is deployed and segments exist (an empty project has nothing to train on; without the voice service there are no models or previews). Training itself lives on the pipeline's Train row; this section holds what training produces:
   ...
   - **Preview** — one text box + conditioning source + shared sampling sliders: **Temperature** (0.05–2, default 0.65), **Speed** (0.5–2, default 1) and **Repetition penalty** (1–20, default 10), with **Top-k** (1–100, default 50) and **Top-p** (0.05–1, default 0.85) behind a collapsed **Advanced sampling** disclosure. All are per-run and shared across both columns, so A/B compares models rather than sampling noise. Then side-by-side zero-shot (base model) and fine-tuned columns generating the same text for A/B listening. Polling for a generating preview is bounded (~10 min) and surfaces a timeout error pointing at the failed-job alerts.
