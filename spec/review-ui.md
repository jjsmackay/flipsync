...
1. **Header** — project name, an **Advanced** checkbox (a per-browser viewing preference persisted in `localStorage`, not project config: knobs flagged `advanced` in the tuning metadata — GPU/VRAM levers, sampling internals, DSP details — render only while it is on; the preview panel's Top-k/Top-p dials follow the same toggle), settings gear (shown only once segments exist; opens the Pipeline section and expands + scrolls to the Review row's Settings disclosure), theme toggle. No review-queue button; review is reached via the stage strip and action card.
...
7. **Models** (v1.5, XTTS) — directly below Pipeline; rendered only when XTTS is deployed and segments exist (an empty project has nothing to train on; without the voice service there are no models or previews). Training itself lives on the pipeline's Train row; this section holds what training produces:
   ...
   - **Preview** — one text box + conditioning source + four shared sampling sliders in a two-column grid: **Temperature** (0.05–2, default 0.65), **Speed** (0.5–2, default 1), **Top-k** (1–100, default 50), **Top-p** (0.05–1, default 0.85). All are per-run and shared across both columns, so A/B compares models rather than sampling noise. `repetition_penalty` is accepted by the API but not a UI dial — the server default (10.0) applies. Then side-by-side zero-shot (base model) and fine-tuned columns generating the same text for A/B listening. Polling for a generating preview is bounded (~10 min) and surfaces a timeout error pointing at the failed-job alerts.
