...
Before a GPU job takes the host-wide GPU lock, the orchestrator waits for its target service to be ready: it polls the service's `GET /health` every 5 seconds, outside the lock, for up to `SERVICE_READY_TIMEOUT_SECS` (default 1800 — generous because diarisation and vocal separation block in startup while models download on first boot). Only once the service reports healthy is the lock acquired and the job submitted; if readiness times out, the job fails with `service_unavailable` without ever holding the lock, so a down service cannot stall every other project's GPU work. CPU jobs are unaffected.
