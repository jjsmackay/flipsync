...
- **Speaker matching method:** Extract a single embedding from the reference clip. For each diarised speaker, compute an average embedding across all their segments. Cosine similarity between reference and each speaker's average determines `match_confidence`. Assign per-speaker confidence to every segment of that speaker.
...
- **On completion**, return `segments` array (id, start_secs, end_secs, speaker_label, match_confidence, wav_path) and `coverage_ratio`.
