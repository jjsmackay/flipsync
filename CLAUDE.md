...
- **Pipeline tuning UI + distinct steps:** Complete on this branch (2026-07-13). Backend: migration-011 knobs were already plumbed; added `GET /sources/{id}/vocals` (stem streaming) and the stage-generic `POST/GET /tuning-preview` endpoints + `tuning_preview` CPU job (ephemeral cleanup render of one segment to `tuning_previews/`, 24 h TTL sweep, status-exempt via `STATUS_EXEMPT_JOB_TYPES`). Frontend: stage strip split to Upload→Speaker→Separate→Match→Transcribe→Review→Export|Train (`deriveStage` rework + `stepChip`); the dashboard is three flat collapsible sections — Sources, Pipeline (the six-row stepper: Separate/Match/Transcribe/Review/Clean & package/Train), and Models (XTTS-gated: trained models + preview, `ModelsSection`) — no Process/Voice wrappers. The Train row (XTTS only) carries a job/model-aware chip, the embedded train panel (button + dataset-mode confirm + per-run Advanced + progress card, no duplicate approved-duration bar) and the persisted xtts_* settings; train navigation and the settings gear target Pipeline rows; models state is dashboard-owned — per-step Settings disclosures PATCHing their own knob subset ("applies when this step re-runs"), re-run-all-eligible-sources with a single invalidation confirm, per-source vocals players, and a cleanup A/B compare modal (two param columns → two tuning previews → save the winner). The Review row is a full step: work-owed chip, segment-count chips + approved-duration bar, review-threshold Settings disclosure, Open review link; the export flow lives on the Clean & package row. There is no top-level Review section any more (StatsPanel deleted); the header gear targets the Review row's disclosure. Knob metadata centralised in `frontend/src/utils/tuning.ts` + shared `KnobFields`/`StageSettingsPanel`; TrainPanel per-run XTTS params, PreviewPanel shared temperature slider, CreateProjectModal Advanced disclosure. TranscribeSettingsPanel absorbed into the Transcribe row. Design doc: `docs/superpowers/specs/2026-07-13-pipeline-tuning-ui-design.md`. Deferred follow-on: separation window A/B (needs service-side window params). Rebased onto the simplify sweep + sep-align upgrades: `tuning_preview` lives in the `job_types.py` registry (`STATUS_EXEMPT_JOB_TYPES` moved there), and the sep-align frontend settings folded into the knob metadata (`demucs_model` select carries all four models with the `htdemucs_ft` default; `align_words` is a Transcribe-row knob).
