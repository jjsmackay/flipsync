...
- **The reference gate lives in `_auto_enqueue_diarisation`.** Step 1 completing without a reference leaves the source at `step2_pending` (not a failure) and the project settles into `awaiting_reference`; `POST /pipeline/continue` resumes. A scout never touches its source's status.
...
- **Next:** Review follow-ups in flight (per-segment match scoring, rejected→pending undo, global GPU semaphore). Outstanding: full end-to-end verification on real GPU hardware (diarisation + steps 2–5, including scout mode, not yet confirmed E2E).
